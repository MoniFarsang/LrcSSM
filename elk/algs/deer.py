...
Taken from https://github.com/lindermanlab/elk/blob/main/elk/algs/deer.py
"""
...
def diagonal_deer_iteration_helper(
    ...
    if dtype in [jnp.complex64, jnp.complex128]:
        jacfunc = jax.vmap(
            jax.jacfwd(func, argnums=0, holomorphic=True), in_axes=(0, 0, None)
    ...
    if dtype == jnp.complex64:
        err = jnp.array(1e10, dtype=jnp.float32)  # initial error should be very high
    elif dtype == jnp.complex128:
        err = jnp.array(1e10, dtype=jnp.float64)
    else:
        err = jnp.array(1e10, dtype=dtype)  # initial error should be very high
    gt = jnp.zeros(
